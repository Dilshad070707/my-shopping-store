...
function StoreApp() {
  ...
  function addToCart(product, requestedSize = "", requestedColor = "") {
    ...
    const price = colorEntry?.price !== null && colorEntry?.price !== undefined && colorEntry?.price !== ""
      ...
      : sizeEntry ? getSizePrice(product, cleanSize) : Number(product.price || 0);

    const cartKey = `${product.id}__${cleanColor || "NOCOLOR"}__${cleanSize || "NOSIZE"}`;
    setCart((current) => {
      const existing = current.find((item) => (item.cartKey || `${item.id}__${item.color || "NOCOLOR"}__${item.size || "NOSIZE"}`) === cartKey);
      if (existing) {
        return current.map((item) => (item.cartKey || `${item.id}__${item.color || "NOCOLOR"}__${item.size || "NOSIZE"}`) === cartKey
          ? { ...item, qty: Math.min(Number(item.qty) + 1, availableStock), stock: availableStock, price }
          : item);
      }
      return [...current, { ...product, cartKey, color: cleanColor || null, size: cleanSize || null, price, stock: availableStock, qty: 1 }];
    });
    notify(cleanColor ? `${cleanColor}${cleanSize ? ` / Size ${cleanSize}` : ""} added to cart.` : "Added to cart.");
  }

  function buyNow(product, requestedSize = "", requestedColor = "") {
    const sizes = getProductSizes(product);
    const cleanSize = String(requestedSize || "").trim().toUpperCase();
    const cleanColor = String(requestedColor || "").trim();
    const colorEntry = Array.isArray(product.colors) ? product.colors.find((entry) => String(entry.name || "").trim() === cleanColor) : null;
    ...
    const price = colorEntry?.price !== null && colorEntry?.price !== undefined && colorEntry?.price !== ""
      ? Number(colorEntry.price) : sizeEntry ? getSizePrice(product, cleanSize) : Number(product.price || 0);
    const cartKey = `${product.id}__${cleanColor || "NOCOLOR"}__${cleanSize || "NOSIZE"}`;
    const item = { ...product, cartKey, color: cleanColor || null, size: cleanSize || null, price, stock: availableStock, qty: 1 };
    setCart((current) => {
      const other = current.filter((x) => (x.cartKey || `${x.id}__${x.color || "NOCOLOR"}__${x.size || "NOSIZE"}`) !== cartKey);
      return [...other, item];
    });
  ...
  function removeFromCart(
    ...
    const cartKey = `${id}__${String(color || "").trim() || "NOCOLOR"}__${String(size || "").toUpperCase() || "NOSIZE"}`;
    setCart(
      (current) =>
        current.filter(
          (item) =>
            (item.cartKey || `${item.id}__${item.color || "NOCOLOR"}__${item.size || "NOSIZE"}`) !== cartKey
        )
    );
  ...
  async function openProduct(
    ...
    try {
      const data = await apiFetch(`/api/products/${encodeURIComponent(product.id)}`);
      if (data.product && String(data.product.id) === String(product.id)) {
        ...
        try {
          const suggestionData = await apiFetch(`/api/products/${encodeURIComponent(product.id)}/suggestions`);
  ...
  return (
    <div className="store">
      ...
      {!filteredBrowse && <section className="hero">
      ...
      {!filteredBrowse && <section className="trust-strip">
      ...
      {!filteredBrowse && <section className="category-showcase">
      ...
      {!filteredBrowse && <section className="deals-section">
        <div className="section-title">
      ...
      {!filteredBrowse && <section className="home-section slider-section" id="best-selling">
        <div className="section-title">
                ...
                <div className="detail-info">
                  ...
                  <div className="detail-buy detail-buy-top">
                    <button
                      ...
                      onClick={() => buyNow(selectedProduct, selectedSize, selectedColor)}
                    >
                      ⚡ BUY NOW
                    </button>
                    <button
              ...
              <div className="cart-items">
                {cart.map(
                  (item) => (
                    <div
                      className="cart-item"
                      key={item.cartKey || `${item.id}__${item.color || "NOCOLOR"}__${item.size || "NOSIZE"}`}
                    >
                      <img
                        src={
                          item.images?.[0]
                      ...
                      <div>
                        ...
                        {item.color && <small className="cart-size">Color: {item.color}</small>}{item.size && <small className="cart-size">Size: {item.size}</small>}

                        <div className="qty">
                          <button
                            onClick={() =>
                              changeQuantity(item.id, -1, item.size, item.color)
                            }
                          >
                            −
                          </button>
...
function AdminApp() {
  ...
  async function handleHomepageImageFile(event) {
    const file = event.target.files?.[0];
    if (!file) return;
    try {
      setFormLoading(true);
      const compressed = await compressProductImage(file);
      setHomepageAdmin((current) => ({ ...current, offer_image: compressed }));
          ...
          {tab === "homepage" && (
            <section>
              ...
              <div className="admin-card">
                ...
                <div className="field"><label>Or Upload Banner Image</label><input type="file" accept="image/*" onChange={handleHomepageImageFile} /><small className="muted">Image is compressed in your browser before saving.</small></div>
