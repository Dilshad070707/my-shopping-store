...
const HOMEPAGE_CACHE_TTL_MS = 30000;
...
app.get(
  ...
  async (req, res) => {
    try {
      ...
      if (search) {
        params.push(`%${search}%`);
        where.push(`(name ILIKE $${params.length} OR category ILIKE $${params.length} OR COALESCE(sku, '') ILIKE $${params.length})`);
      }
      ...
      const countKey = JSON.stringify({ category: category.toLowerCase(), search: search.toLowerCase() });
      const cachedCount = publicProductCountCache.get(countKey);
      const nowForCount = Date.now();

      const [result, countResult] = await Promise.all([
        pool.query(
          `SELECT id, name, sku, category, price, mrp, discount, rating, reviews, stock, sizes, images, description, active, best_selling, new_arrival, featured, created_at, updated_at
           FROM store_products
...
app.get("/api/homepage", async (req, res) => {
  try {
    ...
    const settings = await getSetting("homepage", {
      ...
      offer_image: "", best_selling_title: "🔥 Best Selling Products",
    ...
    const columns = `id, name, sku, category, price, mrp, discount, rating, reviews, stock, sizes, images, description, active, best_selling, new_arrival, featured, created_at, updated_at`;
    const [best, featured, arrivals] = await Promise.all([
      pool.query(`SELECT ${columns} FROM store_products WHERE active = TRUE AND best_selling = TRUE ORDER BY updated_at DESC, created_at DESC LIMIT 20`),
      pool.query(`SELECT ${columns} FROM store_products WHERE active = TRUE AND featured = TRUE ORDER BY updated_at DESC, created_at DESC LIMIT 20`),
      pool.query(`SELECT ${columns} FROM store_products WHERE active = TRUE AND new_arrival = TRUE ORDER BY created_at DESC LIMIT 20`)
    ]);
  ...
  async (req, res) => {
    ...
    try {
      ...
      for (
        ...
        const unitPrice = selectedSize?.price === null || selectedSize?.price === undefined
