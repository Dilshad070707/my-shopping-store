...
app.use(
  express.json({
    // Product photos are compressed in the browser and sent as data URLs.
    // Allow multiple photos in one admin request.
    limit: "50mb",
  })
);
...
function sanitizeImages(images) {
  ...
  return images
    ...
    .filter(Boolean)
    .filter((image) => {
      // Legacy HTTP image URLs remain supported, while new admin uploads
      // are stored as compressed data:image/* base64 strings.
      const isDataImage = /^data:image\/(jpeg|jpg|png|webp|gif);base64,/i.test(image);
      const isHttpImage = /^https?:\/\//i.test(image);
      return isDataImage || isHttpImage;
    })
    .filter((image) => image.length <= 12 * 1024 * 1024)
    .slice(0, 200);
}
