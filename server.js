...
const MAIL_FROM = String(process.env.MAIL_FROM || "MeeshooShopping <onboarding@resend.dev>").trim();
...
async function initializeDatabase() {
  ...
  `);
  await pool.query(`CREATE INDEX IF NOT EXISTS customer_email_otps_email_index ON customer_email_otps(email, created_at DESC)`);
...
function otpHash(email, code) {
  return crypto.createHash("sha256").update(`${email}:${code}:${JWT_SECRET}`).digest("hex");
}
...
app.post("/api/auth/email-login", async (req, res) => {
  try {
    ...
    if (!RESEND_API_KEY) return res.status(503).json({ message: "Email OTP is not configured on the server. Add RESEND_API_KEY and MAIL_FROM." });
    const recent = await pool.query(`SELECT COUNT(*)::int AS count FROM customer_email_otps WHERE email=$1 AND created_at > CURRENT_TIMESTAMP - INTERVAL '10 minutes'`, [email]);
    if (Number(recent.rows[0]?.count || 0) >= 5) return res.status(429).json({ message: "Too many OTP requests. Please try again later." });
    const code = String(crypto.randomInt(100000, 1000000));
    await pool.query(`UPDATE customer_email_otps SET used=TRUE WHERE email=$1 AND used=FALSE`, [email]);
    await pool.query(`INSERT INTO customer_email_otps (email, code_hash, expires_at) VALUES ($1,$2,CURRENT_TIMESTAMP + INTERVAL '10 minutes')`, [email, otpHash(email, code)]);
    try { await sendCustomerOtp(email, code); }
    catch (mailError) { await pool.query(`DELETE FROM customer_email_otps WHERE email=$1 AND code_hash=$2`, [email, otpHash(email, code)]); throw mailError; }
    return res.json({ success: true, otp_required: true, email });
  } catch (error) {
    console.error("Email OTP request error:", error.message);
    return res.status(error.statusCode || 500).json({ message: error.message || "Unable to send OTP." });
  }
});
...
app.post("/api/auth/verify-otp", async (req, res) => {
  try {
    const email = normalizeEmail(req.body.email);
    const code = String(req.body.otp || "").replace(/\D/g, "").slice(0, 6);
    if (!validateEmail(email) || !/^\d{6}$/.test(code)) return res.status(400).json({ message: "Enter the 6-digit OTP sent to your email." });
    const result = await pool.query(`SELECT * FROM customer_email_otps WHERE email=$1 AND used=FALSE ORDER BY created_at DESC LIMIT 1`, [email]);
    if (!result.rows.length) return res.status(400).json({ message: "OTP expired or not found. Request a new OTP." });
    const row = result.rows[0];
    if (new Date(row.expires_at).getTime() <= Date.now()) { await pool.query(`UPDATE customer_email_otps SET used=TRUE WHERE id=$1`, [row.id]); return res.status(400).json({ message: "OTP expired. Request a new OTP." }); }
    if (Number(row.attempts || 0) >= 5) { await pool.query(`UPDATE customer_email_otps SET used=TRUE WHERE id=$1`, [row.id]); return res.status(429).json({ message: "Too many incorrect OTP attempts. Request a new OTP." }); }
    if (otpHash(email, code) !== row.code_hash) { await pool.query(`UPDATE customer_email_otps SET attempts=attempts+1 WHERE id=$1`, [row.id]); return res.status(400).json({ message: "Incorrect OTP." }); }
    await pool.query(`UPDATE customer_email_otps SET used=TRUE WHERE id=$1`, [row.id]);
    const userResult = await pool.query(`INSERT INTO store_users (email) VALUES ($1) ON CONFLICT (email) DO UPDATE SET email=EXCLUDED.email RETURNING id,email,name,phone,created_at`, [email]);
    return res.json({ success:true, verified:true, user:userResult.rows[0] });
  } catch (error) { console.error("OTP verify error:", error.message); return res.status(500).json({ message:"Unable to verify OTP." }); }
});

/* =========================================================
   PUBLIC PRODUCTS — STRICT PUBLIC CATALOG
   ========================================================= */
const PUBLIC_CATEGORIES = ["All","Women","Men","Electronics","Beauty","Footwear","Home","Kitchen","Accessories","Kids","Sports","Bags","Jewellery","Watches","Mobiles","Mobile Accessories","Laptops","Laptop Accessories"];
const CATEGORY_ALIASES = {
  women:["women","woman","ladies","lady","female"], men:["men","man","gents","gentlemen","male"], kids:["kids","kid","children","child"],
  footwear:["footwear","shoe","shoes","slipper","slippers","sandal","sandals","sneaker","sneakers","heel","heels","boot","boots"],
  watches:["watch","watches","smartwatch","smart watch"], mobiles:["mobile","mobiles","phone","phones","smartphone","smartphones","iphone"],
  laptops:["laptop","laptops","notebook","notebooks","macbook"], electronics:["electronics","electronic"], home:["home","home decor","homedecor","home decoration"],
  beauty:["beauty"], kitchen:["kitchen"], accessories:["accessories"], sports:["sports","sport"], bags:["bags","bag"], jewellery:["jewellery","jewelry"],
  "mobile accessories":["mobile accessories","mobile accessory"], "laptop accessories":["laptop accessories","laptop accessory"]
};
function canonicalPublicCategory(value) {
  const s=String(value||"").trim().toLowerCase();
  if (!s || s === "all") return "All";
  const found=PUBLIC_CATEGORIES.find(c=>c.toLowerCase()===s);
  if (found) return found;
  if (["woman","ladies","lady","female"].includes(s)) return "Women";
  if (["man","gents","gentlemen","male"].includes(s)) return "Men";
  if (["watch"].includes(s)) return "Watches";
  if (["mobile","phone","phones","smartphone"].includes(s)) return "Mobiles";
  if (["laptop","notebook","macbook"].includes(s)) return "Laptops";
  if (["shoe","shoes"].includes(s)) return "Footwear";
  return String(value||"").trim();
}
function categorySql(category, params) {
  const c=canonicalPublicCategory(category), lc=c.toLowerCase();
  if (c === "All") return null;
  if (["Women","Men","Kids"].includes(c)) { params.push(c); return `(LOWER(COALESCE(gender,''))=LOWER($${params.length}) OR LOWER(category)=LOWER($${params.length}))`; }
  if (lc === "footwear" || lc === "watches" || lc === "mobiles" || lc === "laptops") { const aliases=CATEGORY_ALIASES[lc]; params.push(aliases); params.push(aliases.map(x=>`%${x}%`)); const a=`$${params.length-1}`, n=`$${params.length}`; return `(LOWER(category) = ANY(${a}::text[]) OR LOWER(COALESCE(subcategory,'')) = ANY(${a}::text[]) OR LOWER(name) ILIKE ANY(${n}::text[]))`; }
  if (lc === "mobile accessories" || lc === "laptop accessories") { const aliases=CATEGORY_ALIASES[lc]; params.push(aliases); return `(LOWER(category) = ANY($${params.length}::text[]) OR LOWER(COALESCE(subcategory,'')) = ANY($${params.length}::text[]))`; }
  const aliases=CATEGORY_ALIASES[lc] || [lc]; params.push(aliases); return `(LOWER(category) = ANY($${params.length}::text[]) OR LOWER(COALESCE(subcategory,'')) = ANY($${params.length}::text[]))`;
}

app.get("/api/products", async (req,res)=>{
  try {
    const rawCategory=String(req.query.category||"").trim();
    const category=canonicalPublicCategory(rawCategory);
    ...
    const page=Math.max(1, Number.isInteger(Number(req.query.page)) ? Number(req.query.page) : 1);
    const pageSize=Math.min(60, Math.max(12, Number.isInteger(Number(req.query.limit)) ? Number(req.query.limit) : 40));
    const cacheKey=JSON.stringify({category,search:normalizedSearch,sort,page,pageSize});
    const now=Date.now(), cached=publicProductsCache.get(cacheKey);
    if(cached && now-cached.time<PRODUCT_CACHE_TTL_MS) return res.json(cached.data);
    const params=[]; const where=["active=TRUE"];
    const catClause=categorySql(category,params); if(catClause) where.push(catClause);
    const searchAliases={
      women:CATEGORY_ALIASES.women, ladies:CATEGORY_ALIASES.women, men:CATEGORY_ALIASES.men, gents:CATEGORY_ALIASES.men,
      watch:CATEGORY_ALIASES.watches, watches:CATEGORY_ALIASES.watches, shoe:CATEGORY_ALIASES.footwear, shoes:CATEGORY_ALIASES.footwear, footwear:CATEGORY_ALIASES.footwear,
      mobile:CATEGORY_ALIASES.mobiles, mobiles:CATEGORY_ALIASES.mobiles, phone:CATEGORY_ALIASES.mobiles, phones:CATEGORY_ALIASES.mobiles,
      laptop:CATEGORY_ALIASES.laptops, laptops:CATEGORY_ALIASES.laptops,
      cloth:["cloth","clothes","clothing","kurti","kurtis","shirt","shirts","jeans","dress","dresses","top","tops","saree","sarees","tshirt","t-shirt","fashion"],
      clothing:["cloth","clothes","clothing","kurti","kurtis","shirt","shirts","jeans","dress","dresses","top","tops","saree","sarees","tshirt","t-shirt","fashion"]
    };
    for(const term of terms){
      const alts=searchAliases[term] || [term]; const clauses=[];
      for(const alt of alts){ params.push(`%${alt}%`); const p=`$${params.length}`; clauses.push(`(name ILIKE ${p} OR category ILIKE ${p} OR subcategory ILIKE ${p} OR gender ILIKE ${p} OR COALESCE(description,'') ILIKE ${p})`); }
      where.push(`(${clauses.join(" OR ")})`);
    }
    let orderBy="best_selling DESC, rating DESC, created_at DESC";
    if(sort==="price-low") orderBy="price ASC, created_at DESC"; else if(sort==="price-high") orderBy="price DESC, created_at DESC"; else if(sort==="rating") orderBy="rating DESC, reviews DESC, created_at DESC"; else if(sort==="discount") orderBy="discount DESC, created_at DESC"; else if(sort==="new-arrival") orderBy="created_at DESC";
    const countKey=JSON.stringify({category,search:normalizedSearch}); const cachedCount=publicProductCountCache.get(countKey); let total;
    if(cachedCount && now-cachedCount.time<PRODUCT_CACHE_TTL_MS) total=cachedCount.count;
    else { const cr=await pool.query(`SELECT COUNT(*)::int AS count FROM store_products WHERE ${where.join(" AND ")}`, params); total=Number(cr.rows[0]?.count||0); publicProductCountCache.set(countKey,{time:now,count:total}); }
    const dataParams=[...params,pageSize,(page-1)*pageSize];
    const result=await pool.query(`SELECT id,name,sku,slug,category,gender,subcategory,price,mrp,discount,rating,reviews,stock,sizes,colors,variants,specifications,manufacturer_info,warranty,images,description,active,best_selling,new_arrival,featured,created_at,updated_at FROM store_products WHERE ${where.join(" AND ")} ORDER BY ${orderBy} LIMIT $${dataParams.length-1} OFFSET $${dataParams.length}`,dataParams);
    const data={success:true,products:result.rows.map(productFromRow),total,page,pageSize,totalPages:Math.max(1,Math.ceil(total/pageSize))};
    publicProductsCache.set(cacheKey,{time:now,data}); return res.json(data);
  } catch(error){ console.error("Products error:",error.message); return res.status(500).json({message:"Unable to load products."}); }
});

app.get("/api/products/:id", async (req,res)=>{
  try { const id=String(req.params.id||"").trim(); if(!id) return res.status(400).json({message:"Product id is required."}); const result=await pool.query(`SELECT * FROM store_products WHERE active=TRUE AND (id=$1 OR LOWER(slug)=LOWER($1) OR LOWER(sku)=LOWER($1)) LIMIT 1`,[id]); if(!result.rows.length) return res.status(404).json({message:"Product not found."}); return res.json({product:productFromRow(result.rows[0])}); }
  catch(error){ console.error("Product detail error:",error.message); return res.status(500).json({message:"Unable to load product details."}); }
});
app.get("/api/products/:id/suggestions", async (req,res)=>{
  try { const id=String(req.params.id||"").trim(); const current=await pool.query(`SELECT id,category FROM store_products WHERE active=TRUE AND (id=$1 OR LOWER(slug)=LOWER($1) OR LOWER(sku)=LOWER($1))`,[id]); if(!current.rows.length) return res.json({products:[]}); const result=await pool.query(`SELECT * FROM store_products WHERE active=TRUE AND LOWER(category)=LOWER($1) AND id<>$2 ORDER BY best_selling DESC,rating DESC,updated_at DESC,created_at DESC LIMIT 12`,[current.rows[0].category,current.rows[0].id]); return res.json({products:result.rows.map(productFromRow)}); }
  catch(error){ console.error("Suggestions error:",error.message); return res.status(500).json({message:"Unable to load suggestions."}); }
});
...
app.get("/api/categories", async (req,res)=>{
  return res.json({success:true,categories:PUBLIC_CATEGORIES});
});
