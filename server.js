  ...
  async (req, res) => {
    try {
      ...
      const [result, countResult] = await Promise.all([
        pool.query(
          `SELECT id, name, sku, category, price, mrp, discount, rating, reviews, stock, sizes,
                  CASE
...
app.get("/api/products/:id", async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `SELECT * FROM store_products WHERE id = $1 AND active = TRUE LIMIT 1`,
      [id]
    );
...
app.get("/api/products/:id/suggestions", async (req, res) => {
  try {
    const id = String(req.params.id || "").trim();
    const current = await pool.query(`SELECT category FROM store_products WHERE id = $1 AND active = TRUE`, [id]);
    if (!current.rows.length) return res.json({ products: [] });
    const result = await pool.query(`SELECT * FROM store_products WHERE active = TRUE AND LOWER(category) = LOWER($1) AND id <> $2 ORDER BY best_selling DESC, rating DESC, updated_at DESC, created_at DESC LIMIT 12`, [current.rows[0].category, id]);
...
app.get("/api/homepage", async (req, res) => {
  try {
    ...
    const columns = `id, name, sku, category, price, mrp, discount, rating, reviews, stock,
      CASE
        WHEN cardinality(images) > 0
        THEN ARRAY[images[1]]
        ELSE ARRAY[]::TEXT[]
