...
app.get("/api/products/:id", async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `SELECT id, name, sku, category, price, mrp, discount, rating, reviews, stock, sizes, images, description,
...
app.get("/api/homepage", async (req, res) => {
  try {
    ...
    const [best, featured, arrivals] = await Promise.all([
      pool.query(`SELECT ${columns} FROM store_products WHERE active = TRUE AND best_selling = TRUE ORDER BY updated_at DESC, created_at DESC LIMIT 12`),
