...
function StoreApp() {
  ...
  useEffect(() => {
    ...
    // First-time visitors must continue through email login.
    // If an old/corrupt saved user value exists, clear it and require login too.
    ...
    if (!parsedSavedUser) setLoginOpen(true);
  ...
  function normalizeSearchForApi(value) {
    const aliases = {
      ladies: "women",
      lady: "women",
      woman: "women",
  ...
  async function loadProducts(showLoading = true) {
    try {
      ...
      const apiSearch = normalizeSearchForApi(search);
  ...
  async function loadCategories() {
    // Keep the public category menu fixed and clean.
    // Database-only/duplicate categories must never be added to the UI.
  ...
  const parsedDiscount = Number(homepage.settings.offer_discount);
  const bannerDiscount = Number.isFinite(parsedDiscount) && parsedDiscount >= 1
    ? Math.min(100, Math.round(parsedDiscount))
    : Number((bannerText.match(/(?:up\s*to\s*)?(\d{1,3})\s*%/i) || [])[1] || 90);

  return (
    <div className="store">
      <style>{STORE_CSS}</style>

      {homepage.settings.show_top_offer_bar !== false && <div className="topbar">
          ...
          <div className="hero-art">
            {(homepage.settings.desktop_banner || homepage.settings.mobile_banner || homepage.settings.offer_image) ? (
              <picture>
                ...
                <div className="hero-image-discount"><small>UP TO</small><strong>{bannerDiscount}%</strong><b>OFF</b></div>
              </picture>
            ) : (
              <>
        ...
        <div className="offer-overlay" onClick={() => setOfferOpen(false)}>
          <div className="offer-popup" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="offer-body">
              ...
              <div className="offer-popup-discount">UP TO {bannerDiscount}% OFF</div>
              <h2>{homepage.settings.popup_title || "Special Offer Just For You"}</h2>
      ...
      {loginOpen && (
        <Modal
          ...
          close={() => {
            ...
            else notify("Please continue with your email to enter the store.");
          }}
          small
        >
          <form
...
function AdminApp() {
  ...
  const [homepageAdmin, setHomepageAdmin] = useState({
    ...
    popup_title: "Special Offer Just For You",
    show_top_offer_bar: true,
            ...
            <section>
              ...
              <div className="admin-card">
                <div className="form-grid">
                  ...
                  <AdminField label="Popup Title" name="popup_title" value={homepageAdmin.popup_title || ""} onChange={(e)=>setHomepageAdmin(v=>({...v,popup_title:e.target.value}))} />
                  <AdminField label="Offer Discount (%)" name="offer_discount" type="number" min="1" max="100" value={homepageAdmin.offer_discount ?? 90} onChange={(e)=>setHomepageAdmin(v=>({...v,offer_discount:Math.min(100, Math.max(1, Number(e.target.value || 90)))}))} />
                ...
                <div className="field"><label>Offer Popup Image — Direct Photo Upload</label><input type="file" accept="image/*" onChange={(e)=>handleHomepageImageFile(e,"offer_image")} disabled={formLoading}/><small className="muted">Popup में दिखने वाली offer photo.</small>{homepageAdmin.offer_image && <img className="admin-banner-preview" src={homepageAdmin.offer_image} alt="Offer popup preview"/>}</div>
                ...
                <div className="admin-control-grid">
                  <label className="checkbox-row"><input type="checkbox" checked={homepageAdmin.show_top_offer_bar !== false} onChange={(e)=>setHomepageAdmin(v=>({...v,show_top_offer_bar:e.target.checked}))}/> Show top offer bar</label>
...
.offer-badge { font-size:11px; font-weight:950; letter-spacing:.14em; color:#7c3aed; } .offer-popup-discount{display:inline-block;margin:8px 0 10px;padding:8px 12px;border-radius:999px;background:linear-gradient(135deg,#7c3aed,#db2777);color:#fff;font-size:16px;font-weight:950;letter-spacing:.04em;box-shadow:0 8px 20px rgba(124,58,237,.22);}
...
.hero-image-discount { position:absolute; top:8%; right:7%; width:150px; height:150px; border-radius:50%; display:flex; flex-direction:column; align-items:center; justify-content:center; color:#fff; background:radial-gradient(circle,#ef476f 0 42%,#b40f58 43% 62%,#f8b22d 63% 68%,#9d1258 69%); border:6px solid #ffca57; box-shadow:0 0 0 9px rgba(255,202,87,.12),0 15px 30px rgba(0,0,0,.22); transform:rotate(5deg); z-index:3; text-align:center; } .hero-image-discount small { font-weight:900; font-size:11px; } .hero-image-discount strong { font-size:42px; line-height:.9; } .hero-image-discount b { font-size:19px; } @media(max-width:767px){ .hero-image-discount{width:108px;height:108px;border-width:4px;top:6%;right:5%;} .hero-image-discount strong{font-size:30px;} .hero-image-discount b{font-size:14px;} .hero-image-discount small{font-size:8px;} } .hero-discount small { font-weight:900; font-size:16px; }
...
/* Final mobile-safe layout: phones always get two readable product columns. */
@media (pointer: coarse) and (hover: none) {
  html, body { overflow-x:hidden; }
  .store { width:100%; max-width:100%; overflow-x:hidden; }
  ...
  .product-name { display:-webkit-box; -webkit-box-orient:vertical; -webkit-line-clamp:3; overflow:hidden; min-height:0; }
  .actions { grid-template-columns:1fr !important; }
  .category-showcase-grid, .deals-grid { display:flex; overflow-x:auto; gap:9px; scrollbar-width:none; }
  .category-showcase-grid::-webkit-scrollbar, .deals-grid::-webkit-scrollbar { display:none; }
  .category-tile, .deal-card { flex:0 0 150px; }
  .product-slider { padding-bottom:8px; }
}
...
/* Final storefront overrides: categories stay in one horizontal row. */
.categories {
...
.category { flex:0 0 auto !important; white-space:nowrap; }
.category-showcase-grid {
  display:flex !important;
  flex-wrap:nowrap !important;
  overflow-x:auto !important;
...
.category-tile { flex:0 0 145px !important; }
@media (max-width:767px) {
  .category-showcase-grid { gap:9px !important; }
  .category-tile { flex-basis:132px !important; min-height:82px; }
  .categories { gap:7px; padding-bottom:12px; }
  .category { padding:9px 13px; font-size:12px; }
}
@media (min-width:768px) {
  .category-showcase-grid { gap:13px !important; }
  .category-tile { flex-basis:155px !important; }
}
